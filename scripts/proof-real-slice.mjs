...
// This file's body is top-level, so `import`ing it for a helper would silently start a real
// generation: acquire the GPU, load a 14 GB model and render. It exports nothing, so refuse.
// (Found the hard way — an `import()` of this path ran the whole proof.)
if (
  process.argv[1] === undefined ||
  path.resolve(process.argv[1]) !== fileURLToPath(import.meta.url)
) {
  throw new Error(
    'proof-real-slice.mjs is an executable proof, not a module: importing it would start a real GPU run',
  )
}
