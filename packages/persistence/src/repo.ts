...
import { AudiobookJob, type Book, type AudiobookJobSnapshot, DomainError, OutputVersion } from '@light-novel-audiobook/domain';
import type {
  JobRepository,
...
import { toSafeAbsolute, sha256OfFile, outputBaseName } from './workspace.js';
...
export class SqliteJobRepository implements JobRepository {
  ...
  async findJob(jobId: string): Promise<AudiobookJob | undefined> {
    ...
    const row = this.db
      ...
      .get(jobId) as { snapshot_json?: string } | null;
  ...
  async saveBook(book: Book): Promise<void> {
    const tx = this.db.transaction(() => {
      ...
      for (const chapter of book.chapters) {
        this.db
          .prepare(
            'INSERT INTO chapters (id, book_id, position, title) VALUES (?, ?, ?, ?)',
          )
  ...
  async reserveNextOutput(book: Book): Promise<OutputReservation> {
    ...
    const existing = this.db
      ...
      .get(book.id) as { v: number | null } | null;
    ...
    const tx = this.db.transaction(() => {
      while (true) {
        ...
        this.db
          ...
          .run(book.id, candidate, m4bPathLocked, chapterPathsJson);

        // Ensure output directories exist
        mkdirSync(dirname(m4bPathLocked), { recursive: true });
