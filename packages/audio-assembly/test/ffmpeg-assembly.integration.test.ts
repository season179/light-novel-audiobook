...
/**
 * This is the only suite that spawns the real binaries, so it is the only one that depends on the
 * pinned toolchain being installed. The probe is a plain existence check rather than
 * `resolveFfmpegToolchain`, because resolving is what throws — and a throw here would fail a host
 * that simply has no toolchain instead of reporting missing coverage. It follows the same
 * `skipIf(!existsSync(...))` shape the committed-evidence suite uses. Resolution still honours
 * ${FFMPEG_DIRECTORY_ENV}, so a job that does install the toolchain runs the suite in full,
 * including the strict version pin.
 */
const TOOLCHAIN_DIRECTORY = resolve(process.env[FFMPEG_DIRECTORY_ENV] ?? defaultFfmpegDirectory())
const TOOLCHAIN_PRESENT =
  existsSync(join(TOOLCHAIN_DIRECTORY, 'ffmpeg')) &&
  existsSync(join(TOOLCHAIN_DIRECTORY, 'ffprobe'))
const SKIP_REASON = `pinned ffmpeg/ffprobe not found in ${TOOLCHAIN_DIRECTORY}`

if (!TOOLCHAIN_PRESENT) {
  // Skipped coverage has to announce itself or it disappears from CI without anyone noticing.
  // Written straight to stderr on purpose: Vitest swallows `console` output from a file whose tests
  // are all skipped, and the summary alone would report the skip without ever saying why.
  process.stderr.write(
    `[skipped] real-ffmpeg assembly coverage did not run: ${SKIP_REASON}.\n` +
...
describe.skipIf(!TOOLCHAIN_PRESENT)(
  TOOLCHAIN_PRESENT
    ? 'FFmpeg assembly against real audio'
    : `FFmpeg assembly against real audio [SKIPPED: ${SKIP_REASON}]`,
  () => {
    ...
    it(
      ...
      async () => {
        const before = await Promise.all(
          [result.m4bPath, result.manifestPath, ...result.chapters.map((c) => c.path)].map(
            sha256Of,
          ),
        )
        const assembler = await FfmpegAudioAssembler.create()
