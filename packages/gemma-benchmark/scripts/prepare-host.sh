...
data_root="${XDG_DATA_HOME:-$HOME/.local/share}/light-novel-audiobook"
tts_runtime_root="${LIGHT_NOVEL_AUDIOBOOK_TTS_RUNTIME_ROOT:-$data_root/runtimes/tts}"
tts_model_root="${LIGHT_NOVEL_AUDIOBOOK_TTS_MODEL_ROOT:-$data_root/models/tts}"
tts_workspace_root="${LIGHT_NOVEL_AUDIOBOOK_TTS_WORKSPACE_ROOT:-$data_root/workspaces/spikes/issue-7}"
...
reject_symlinks() {
  ...
  for part in "${parts[@]}"; do
    ...
    [[ ! -L "$current" ]] || { printf 'Refusing symlink component: %s\n' "$current" >&2; exit 1; }
  done
}
...
for protected in "$repo_root" "$git_dir" "$tts_runtime_root" "$tts_model_root" "$tts_workspace_root"; do
  protected="$(realpath -m "$protected")"
  if overlap "$runtime_root" "$protected"; then
    printf 'Brain runtime overlaps Git or protected TTS storage.\n' >&2
    exit 1
  fi
done
probe="$runtime_root"
while [[ ! -e "$probe" ]]; do
  ...
  [[ "$parent" != "$probe" ]] || { printf 'No existing runtime ancestor.\n' >&2; exit 1; }
  probe="$parent"
done
[[ "$(findmnt -n -o FSTYPE -T "$(realpath "$probe")")" == ext4 ]] || {
  printf 'Runtime must use ext4.\n' >&2
  exit 1
}

source_dir="$runtime_root/llama.cpp"
source_part="$runtime_root/.llama.cpp.prepare"
...
card_path="$evidence_dir/model-card.md"
gitattributes_path="$evidence_dir/gitattributes"
apache_path="$evidence_dir/Apache-2.0.txt"
llama_license_path="$evidence_dir/llama.cpp-LICENSE"
manifest="$runtime_root/host-build.json"

# Guard every owned destination and temporary target before the first mutation.
for path in \
  "$runtime_root" "$source_dir" "$source_part" "$binary" "$model_dir" "$model_path" \
  "$model_path.part" "$evidence_dir" "$api_path" "$api_path.part" "$headers" "$headers.part" \
  "$card_path" "$card_path.part" "$gitattributes_path" "$gitattributes_path.part" \
  "$apache_path" "$apache_path.part" "$llama_license_path" "$llama_license_path.part" \
  "$manifest" "$manifest.part"; do
  reject_symlinks "$path"
done
if [[ "${GEMMA_BENCHMARK_GUARD_ONLY:-0}" == 1 ]]; then
  printf 'External brain path guards passed before mutation.\n'
  exit 0
fi
...
fetch_hashed "https://huggingface.co/$model_repo/raw/$model_revision/README.md" "$card_path" "$card_sha"
fetch_hashed "https://huggingface.co/$model_repo/raw/$model_revision/.gitattributes" "$gitattributes_path" "$gitattributes_sha"
fetch_hashed 'https://www.apache.org/licenses/LICENSE-2.0.txt' "$apache_path" "$apache_license_sha"
fetch_hashed "https://raw.githubusercontent.com/ggml-org/llama.cpp/$llama_commit/LICENSE" "$llama_license_path" "$llama_license_sha"
...
rm -rf "$source_part"
git init --quiet "$source_part"
git -C "$source_part" remote add origin "$llama_repo"
git -C "$source_part" fetch --depth 1 origin "$llama_commit"
git -C "$source_part" checkout --quiet --detach FETCH_HEAD
rm -rf "$source_dir"
mv "$source_part" "$source_dir"
