...
export const createGenerationCommandIdentity = (input: GenerationCommandIdentityInput): string => {
  if (input.epubPath.length === 0) throw new DomainError('EPUB path is required')
  ...
  if (externalIdentities.some((identity) => identity.length === 0)) {
    throw new DomainError('Director, speech, and assembler identities are required')
  }
