...
runtime_root="$(realpath -m "$runtime_root_input")"
...
if [[ -L "$runtime_root" || -L "$source_dir" || -L "$model_dir" ]]; then
  printf 'Runtime, source, and model directories must not be symlinks\n' >&2
  exit 1
fi

cleanup_temporary_files() {
  rm -rf -- "$temporary_source"
  rm -f -- "$model_part" "$license_dir"/*.part 2>/dev/null || true
}
...
printf '%s\n' \
  ...
  '  "cleanRebuild": true' \
  '}' > "$runtime_root/host-build.json.tmp"
mv "$runtime_root/host-build.json.tmp" "$runtime_root/host-build.json"
"$binary_path" --version
