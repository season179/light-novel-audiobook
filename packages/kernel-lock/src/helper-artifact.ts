...
const SOURCE_PATH = fileURLToPath(new URL('../native/darwin-held-lock.c', import.meta.url))
const CONFIG_PATH = fileURLToPath(
  new URL('../../../config/kernel-lock-artifacts.json', import.meta.url),
)
...
async function readPin(): Promise<ArtifactPin> {
  const pin = JSON.parse(await readFile(CONFIG_PATH, 'utf8')) as ArtifactPin
  if (
    pin.schemaVersion !== 1 ||
    typeof pin.protocol !== 'string' ||
    !HEX_SHA256.test(pin.source?.sha256 ?? '') ||
    pin.source.path !== 'packages/kernel-lock/native/darwin-held-lock.c' ||
  ...
  const sourceHash = sha256(await readFile(SOURCE_PATH))
...
async function build(pin: ArtifactPin, directory: string, compiler: string): Promise<void> {
  ...
  try {
    await new Promise<void>((resolveBuild, rejectBuild) => {
      const child = spawn(compiler, [...COMPILE_ARGUMENTS, SOURCE_PATH, '-o', temporaryBinary], {
...
export async function resolveVerifiedDarwinHelper(
  ...
  await build(pin, directory, options.compiler ?? process.env.CC ?? '/usr/bin/cc')
  return { path: await verify(pin, directory), protocol: pin.protocol }
}

export const darwinHelperSourcePath = (): string => SOURCE_PATH
export const darwinHelperPinPath = (): string => CONFIG_PATH
